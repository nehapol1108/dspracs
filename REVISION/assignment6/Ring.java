...
public class Ring {
    ...
    public void displayArrayList(ArrayList<Integer>pid){
        for(Integer x:pid){
            System.out.print(x + " ");
        }
    }
    public void initElection(int process_id){
        if(processes[process_id-1]){
            pid.add(process_id);
            int temp = process_id;
            System.out.println("process id " + (process_id) + " sending active list " );
            displayArrayList(pid);
            while(temp!=(process_id-1)){
                if(processes[temp]){
                    pid.add(temp+1);
                    System.out.println("process id " + (temp +1) + " sending active list " );
                    displayArrayList(pid);
                }
